document.addEventListener('DOMContentLoaded', () => {
    // Обновленные цены и типы мест
    const prices = { regular: 500, comfort: 550 };
    let selectedSeats = [];
    ...
    function generateSeats() {
        // Расположение мест как на скриншоте с разными типами
        const rows = [
            { type: 'regular', pattern: '111111000111111' },    // Ряд 1: обычные места
            { type: 'regular', pattern: '11111111111111' },     // Ряд 2: обычные места
            { type: 'regular', pattern: '11111111111111' },     // Ряд 3: обычные места
            { type: 'comfort', pattern: '111111011101111' },    // Ряд 4: комфорт места
            { type: 'comfort', pattern: '11111111111111' },     // Ряд 5: комфорт места
            { type: 'comfort', pattern: '11111111111111' },     // Ряд 6: комфорт места
            { type: 'comfort', pattern: '11111111111111' },     // Ряд 7: комфорт места
            { type: 'comfort', pattern: '111111000011111111' }  // Ряд 8: комфорт места
        ];
    ...
    function updateSelection() {
        selectedList.innerHTML = selectedSeats.map(seat => `
            <div class="seat-item">
                ${seat.row} ряд, ${seat.seat} место (${seat.type === 'regular' ? 'Место' : 'Комфорт'}) - ${seat.price} ₽
